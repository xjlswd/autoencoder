...
class AutoencoderKL(pl.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        reconstructions, posterior = self(batch)
        aeloss, log_dict_ae = self.loss(batch, reconstructions, posterior, 0, self.global_step,
                                        last_layer=self.get_last_layer(), split="val")

        discloss, log_dict_disc = self.loss(batch, reconstructions, posterior, 1, self.global_step,
                                            last_layer=self.get_last_layer(), split="val")
    ...
    def _validation_step(self, batch, batch_idx, suffix=""):
        ...
        aeloss, log_dict_ae = self.loss(qloss, batch, xrec, 0,
                                        self.global_step,
        ...
        discloss, log_dict_disc = self.loss(qloss, batch, xrec, 1,
                                            self.global_step,
                                            last_layer=self.get_last_layer(),
                                            split="val"+suffix,
                                            predicted_indices=ind
